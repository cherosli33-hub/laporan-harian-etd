...
export default function Home() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!draft.filledBy.trim()) return notify("Sila isi nama pengisi dahulu"); persist({ ...draft, id: `${draft.date}_${draft.shift}` }); };
  const remove = (report: Report) => {
    if (!window.confirm(`Padam laporan ${report.shift}, ${formatDate(report.date)}?`)) return;
    localReportRepository.remove(report.id);
    setReports(localReportRepository.getAll());
  ...
  return (
    <main className="app-shell">
      <header className="topbar no-print">
        <div className="brand"><div className="brand-mark" aria-hidden="true">+</div><div><p className="eyebrow">E.T.D HOSPITAL KUALA LIPIS</p><h1>Laporan Harian ETD</h1></div></div>
        <span className="test-badge"><i /> MOD UJIAN</span>
      </header>
      <section className="offline-note no-print"><span>⌁</span><p><strong>Data disimpan pada peranti ini sahaja.</strong> Belum dikongsi dengan telefon lain.</p></section>
