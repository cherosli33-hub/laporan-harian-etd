...
export const localReportRepository = {
  ...
  getDrafts(): Report[] { if (typeof window === "undefined") return []; try { return JSON.parse(window.localStorage.getItem(DRAFT_KEY) || "[]") as Report[]; } catch { return []; } },
  getDraft(id: string) { return this.getDrafts().find((draft) => draft.id === id); },
  saveDraft(report: Report) { const drafts = this.getDrafts(); const index = drafts.findIndex((item) => item.id === report.id); const next = { ...report, updatedAt: new Date().toISOString() }; if (index < 0) drafts.push(next); else drafts[index] = next; window.localStorage.setItem(DRAFT_KEY, JSON.stringify(drafts)); return next; },
  removeDraft(id: string) { window.localStorage.setItem(DRAFT_KEY, JSON.stringify(this.getDrafts().filter((draft) => draft.id !== id))); },
};
